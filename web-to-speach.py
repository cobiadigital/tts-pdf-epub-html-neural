...
with sync_playwright() as p:
    ...
    page.goto("https://www.science.org/content/article/professors-struggle-recruit-postdocs-calls-structural-change-academia-intensify")
    response = page.content()
    browser.close()

response = str(page.content)
...
# with open(filename, 'r', encoding='utf-8') as file:
    ...
    title = soup.find('h1')
    # if nature
    #teaser = soup.find('div', {'class': 'c-article-teaser-text'}).get_text()
    #if nature
    #article = soup.find('article').find("div", {'class': 'c-article-body'})
    article = soup.find('article').find("section", {'class': 'bodySection'})

        #create a new string to parse
    #soup_export = str(title) + '<h2>' + str(teaser) + '</h2>'+ str(article)
    soup_export = str(title) + str(article)

    soup = bs4.BeautifulSoup(soup_export, "lxml")
...
url = Request('http://www.science.org/content/article/professors-struggle-recruit-postdocs-calls-structural-change-academia-intensify', headers={'User-Agent': 'Mozilla/5.0'})
response = urlopen(url).read()
audio_content = synthesize_ssml(client, response, voice=voice, audio_config=audio_config)
mp3_name = 'nature' + "_.mp3"
...
# url = Request('http://www.nature.com/articles/d41586-022-01512-6', headers={'User-Agent': 'Mozilla/5.0'})
# response = urlopen(url).read()
#
# textblocks_output = pollytext(response)
#
#
